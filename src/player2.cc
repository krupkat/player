...
static SDL_Texture *CreateTexture(SDL_Renderer *r, unsigned char *data,
                                  unsigned int len, int *w, int *h) {
  ...
  if (src) {
    ...
    if (surface) {
      ...
      SDL_SetSurfaceColorKey(surface, true,
                             SDL_MapSurfaceRGB(surface, 255, 255, 255));
...
static int InitSprites(void) {
  ...
  sprite =
      CreateTexture(renderer, icon_bmp, icon_bmp_len, &sprite_w, &sprite_h);
...
static void registry_global(void *data, struct wl_registry *wl_registry,
  ...
  if (SDL_strcmp(interface, xdg_wm_base_interface.name) == 0) {
    state.xdg_wm_base = (struct xdg_wm_base*)
        wl_registry_bind(state.wl_registry, name, &xdg_wm_base_interface, 1);
...
int main(int argc, char **argv) {
  ...
  if (SDL_strcmp(SDL_GetCurrentVideoDriver(), "wayland") != 0) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                 "Video driver must be 'wayland', not '%s'",
  ...
  SDL_SetBooleanProperty(
      props, SDL_PROP_WINDOW_CREATE_WAYLAND_SURFACE_ROLE_CUSTOM_BOOLEAN,
      true); /* Roleless surface */
  ...
  SDL_SetBooleanProperty(props,
                         ...
                         true); /* Handle DPI scaling internally */
  SDL_SetStringProperty(props, SDL_PROP_WINDOW_CREATE_TITLE_STRING,
                        "Wayland custom surface role test"); /* Default title */
  ...
  /* Get the display object and use it to create a registry object, which will
   * enumerate the xdg_wm_base protocol. */
  state.wl_display = (struct wl_display*)
  ...
  /* Get the wl_surface object from the SDL_Window, and create a toplevel window
   * with it. */
  state.wl_surface = (struct wl_surface*)
      SDL_GetPointerProperty(SDL_GetWindowProperties(window),
...
exit:
  /* The display and surface handles obtained from SDL are owned by SDL and must
  ...
  if (window) {
    ...
    window = NULL;wl_display_dispatch(state.wl_display);
  }
